...
class TestFSTPut(unittest.TestCase):
    ...
    def test_get_one_special(self):
        ...
        self.assertEqual('{a: b}', f.get(0).src)
        ...
        self.assertEqual('{1: b}', f.get(0).src)
        ...
        self.assertEqual('{c: d}', FST('{a: b, c: d, e: f}').get(1).src)
        # self.assertRaises(ValueError, FST('match a:\n case {1: b}: pass').cases[0].pattern.get, 0)
        self.assertEqual('{2: b}', FST('match a:\n case {1: a, 2: b, 3: c}: pass').cases[0].pattern.get(1).root.src)
        ...
        self.assertEqual('ident', FST('global ident').get(0, 'names', promote=False))
        self.assertEqual('ident', FST('nonlocal ident').get(0, 'names', promote=False))
...
if 1:
    ...
    def test_put_param_matrix(self):
        ...
        self.assertEqual('[[x]]', FST('[a, b, c]').put('[x]', None, None, one=False).src)
        self.assertEqual('[x]', FST('[a, b, c]').put('[x]', None, None, one=None).src)
        self.assertEqual('[x, y]', FST('[a, b, c]').put('x, y', None, None, one=False).src)
        self.assertEqual('[(x, y)]', FST('[a, b, c]').put('x, y', None, None, one=True).src)
        ...
        self.assertEqual('[a, b, c, x]', FST('[a, b, c]').put('[x]', 'end', None, one=None).src)
        self.assertEqual('[a, b, c, [x]]', FST('[a, b, c]').put('[x]', 'end', None, one=False).src)
        self.assertEqual('[a, b, c, x, y]', FST('[a, b, c]').put('x, y', 'end', None, one=None).src)
        self.assertEqual('[a, b, c, x, y]', FST('[a, b, c]').put('x, y', 'end', None, one=False).src)
        self.assertEqual('[a, b, c, (x, y)]', FST('[a, b, c]').put('x, y', 'end', None, one=True).src)
        self.assertEqual('[x, b, c]', FST('[a, b, c]').put('x', None, 1, one=False).src)
        self.assertEqual('[[x], b, c]', FST('[a, b, c]').put('[x]', None, 1, one=False).src)
        self.assertEqual('[x, b, c]', FST('[a, b, c]').put('[x]', None, 1, one=None).src)
        self.assertEqual('[a, x, b, c]', FST('[a, b, c]').put('x', 1, 1, one=False).src)
        self.assertEqual('[a, [x], b, c]', FST('[a, b, c]').put('[x]', 1, 1, one=False).src)
        self.assertRaises(IndexError, FST('[a, b, c]').put,'[x]', 'end', 1)
        self.assertEqual('[x]', FST('[a, b, c]').put('x', None, 'end', one=False).src)
        self.assertEqual('[[x]]', FST('[a, b, c]').put('[x]', None, 'end', one=False).src)
        self.assertEqual('[x]', FST('[a, b, c]').put('[x]', None, 'end', one=None).src)
        self.assertEqual('[a, x]', FST('[a, b, c]').put('x', 1, 'end', one=False).src)
        self.assertEqual('[a, [x]]', FST('[a, b, c]').put('[x]', 1, 'end', one=False).src)
        self.assertEqual('[a, x]', FST('[a, b, c]').put('[x]', 1, 'end', one=None).src)
        self.assertEqual('[a, b, c, x]', FST('[a, b, c]').put('x', 'end', 'end', one=False).src)
        self.assertEqual('[a, b, c, [x]]', FST('[a, b, c]').put('[x]', 'end', 'end', one=False).src)
        self.assertEqual('[a, b, c, x]', FST('[a, b, c]').put('[x]', 'end', 'end', one=None).src)
...
"""''', raw=False)
    ...
    def test__put_one_virtual_field(self):
        ...
        # self.assertRaises(NodeError, FST('{a: b, c: d, e: f}')._put_one, 'x: y', 1, '_all', {'raw': False}, True)
        (f := FST('{a: b, c: d, e: f}'))._put_one('{x: y}', 1, '_all', {'raw': False}, True)
        self.assertEqual('{a: b, x: y, e: f}', f.src)
        ...
        # self.assertRaises(NodeError, FST('{a: b, c: d, e: f}')._put_one, 'x: y', 1, '_all', {'raw': False}, False)
        (f := FST('{a: b, c: d, e: f}'))._put_one('{x: y}', 1, '_all', {'raw': False}, False)
        self.assertEqual('{a: b, x: y, e: f}', f.src)
        ...
        # self.assertRaises(NodeError, FST('case {1: a, 2: b, 3: c}: pass').pattern._put_one, '4: x', 1, '_all', {'raw': False}, True)
        (f := FST('{1: a, 2: b, 3: c}'))._put_one('{4: x}', 1, '_all', {'raw': False}, True)
        self.assertEqual('{1: a, 4: x, 3: c}', f.src)
        ...
        # self.assertRaises(NodeError, FST('case {1: a, 2: b, 3: c}: pass').pattern._put_one, '4: x', 1, '_all', {'raw': False}, False)
        (f := FST('{1: a, 2: b, 3: c}'))._put_one('{4: x}', 1, '_all', {'raw': False}, False)
        self.assertEqual('{1: a, 4: x, 3: c}', f.src)
        ...
        # self.assertRaises(NodeError, FST('match _:\n case {1: a, 2: b, 3: c}: pass').cases[0].pattern._put_one, '4: x', 1, '_all', {'raw': False}, True)
        (f := FST('match _:\n case {1: a, 2: b, 3: c}: pass')).cases[0].pattern._put_one('{4: x}', 1, '_all', {'raw': False}, True)
        self.assertEqual('match _:\n case {1: a, 4: x, 3: c}: pass', f.src)
