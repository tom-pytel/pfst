...
PARSE_TESTS = [
    ...
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    ''),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    'a'),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    'a ='),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    'a = b'),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    'a = b ='),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    '\\\na\\\n = \\\n'),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    ' a'),
    ('Assign_targets',    ep.parse_Assign_targets,    _slice_Assign_targets,    '\na'),
    ...
    ('all',               ep.parse_Assign_targets,    _slice_Assign_targets,    'a = b ='),
...
class TestFST(unittest.TestCase):
    def test_unparse(self):
        ...
        for op in (Add, Sub, Mult, Div, Mod, Pow, LShift, RShift, BitOr, BitXor, BitAnd, FloorDiv, And, Or):
            self.assertEqual(OPCLS2STR[op], ep.unparse(FST(f'a {OPCLS2STR[op]} b').op.a))

        for op in (Add, Sub, Mult, Div, Mod, Pow, LShift, RShift, BitOr, BitXor, BitAnd, FloorDiv):
            self.assertEqual(OPCLS2STR[op], ep.unparse(FST(f'a {OPCLS2STR[op]}= b').op.a))

        for op in (Invert, Not, UAdd, USub):
            self.assertEqual(OPCLS2STR[op], ep.unparse(FST(f'{OPCLS2STR[op]} a').op.a))

        for op in (Eq, NotEq, Lt, LtE, Gt, GtE, Is, IsNot, In, NotIn):
            self.assertEqual(OPCLS2STR[op], ep.unparse(FST(f'a {OPCLS2STR[op]} b').ops[0].a))
