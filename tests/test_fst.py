...
class TestFST(unittest.TestCase):
    ...
    def test_fix(self):
        f = FST.fromsrc('if 1:\n a\nelif 2:\n b')
        fc = f.a.body[0].orelse[0].f.copy(fix=False, raise_=True)
        ...
        f = FST.fromsrc('(1 +\n2)')
        fc = f.a.body[0].value.f.copy(fix=False, raise_=True)
        ...
        fc = f.a.body[0].targets[0].f.copy(fix=False, raise_=True)
        ...
        f = f.a.body[0].cases[0].pattern.f.copy(fix=False, raise_=True)
        self.assertIs(f.a.__class__, MatchValue)
        f.fix(inplace=True, raise_=True)
        self.assertIs(f.a.__class__, MatchValue)
        lines = f._lines
        self.assertIsNot(lines, f.fix(mutate=True, inplace=False, raise_=True)._lines)
        f.fix(mutate=True, inplace=True, raise_=True)
        ...
        f = FST.fromsrc('def f( \n a \n : \n int \n ): pass')
        f = f.a.body[0].args.args[0].f.copy(fix=False, raise_=True)
        ...
        f = FST.fromsrc('f( \n a \n = \n 1 \n )')
        f = f.a.body[0].value.keywords[0].f.copy(fix=False, raise_=True)
        ...
        if sys.version_info[:2] >= (3, 12):
            ...
            f = f.a.body[0].type_params[0].f.copy(fix=False, raise_=True)
    ...
    def test_copy_bulk(self):
        for fnm in PYFNMS:
            ...
            for a in walk(ast):
                if a.f.is_parsable():
                    f = a.f.copy(fix=True, raise_=True)
                    f.verify(raise_=True)

    def test_copy(self):
        f = FST.fromsrc('@decorator\nclass cls:\n  pass')
        self.assertEqual(f.a.body[0].f.copy(fix=False, raise_=True).src, '@decorator\nclass cls:\n  pass')
        self.assertEqual(f.a.body[0].f.copy(decorators=False, fix=False, raise_=True).src, 'class cls:\n  pass')
        ...
        self.assertRaises(ValueError, f.fix)
