...
def func():
    ...
    def test_fix(self):
        ...
        fc._maybe_fix_single_copy(inplace=True)
        ...
        g = f._maybe_fix_single_copy(inplace=False)
        self.assertIsNot(g, f)
        self.assertEqual('if 2: pass', g.src)
        self.assertEqual('elif 2: pass', f.src)

        g = f._maybe_fix_single_copy(inplace=True)
        self.assertIs(g, f)
        self.assertEqual('if 2: pass', g.src)
        ...
        self.assertIsNot(f, f._maybe_fix_single_copy(inplace=False))

        g = f._maybe_fix_single_copy(inplace=False)
        self.assertFalse(compare_asts(f.a, g.a))
        self.assertIs(g, g._maybe_fix_single_copy(inplace=False))
        ...
        fc = f.a.body[0].value.f.copy(fix=False)
        self.assertEqual('1 +\n2', fc.src)
        fd = fc._maybe_fix_single_copy(inplace=False)
        self.assertEqual('(1 +\n2)', fd.src)
        fc._maybe_fix_single_copy(inplace=True)
        self.assertEqual('(1 +\n2)', fc.src)
...
((is_seq := isinstance(a, (Tuple, List))) or (is_starred := isinstance(a, Starred)) or
    ...
    def test_copy(self):
        ...
        self.assertIs(f._maybe_fix_single_copy(inplace=False), f)
        # self.assertRaises(SyntaxError, f.fix)
        # self.assertIs(None, f._maybe_fix_single_copy(raise_=False))
