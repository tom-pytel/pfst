...
from .code import (
    _code_as_one__Assign_targets,
    _code_as_one__decorator_list,
...
# The _code_to_slice_*() functions mostly deal with the `one` parameter, which is tricky because it can mean one of two
# things:
# - Put slice as a single element.
# - Put one element as single-element slice.
# It is also tricky because the nodes which are slices can also be elements of those slices.
...
def _code_to_slice_expr(
    ...
    fst_ = code_as(code, options, self.root._parse_params, coerce=coerce)
    ast_ = fst_.a
    ast_cls = ast_.__class__
    is_pard_tup = fst_.is_parenthesized_tuple()
    is_slice_type = ast_cls in ASTS_LEAF_TUPLE_LIST_OR_SET

    if one is None or not isinstance(code, (str, list)):
        is_slice = is_slice_type
    else:
        is_slice = is_pard_tup is False
    ...
    if not one:
        if is_slice:
            ...
            if is_pard_tup is not False:  # anything that is not an unparenthesize tuple is restricted to the inside of the delimiters, which are removed
    ...
    # one=True or any expression which is not a slice type we can implicitly coerce to a singleton slice

    if not is_slice and not (one or coerce):
    ...
    if is_pard_tup is not None:
        if is_pard_tup is False:  # don't put unparenthesized tuple source as one into sequence, it would merge into the sequence
            fst_._delimit_node()

    elif ast_cls is NamedExpr:  # this needs to be parenthesized if being put to unparenthesized tuple
        if not fst_.pars().n and self.is_parenthesized_tuple() is False:
            fst_._parenthesize_grouping()

    elif ast_cls in (Yield, YieldFrom):  # these need to be parenthesized definitely
        if not fst_.pars().n:
            fst_._parenthesize_grouping()

    ast_ = Tuple(elts=[ast_], ctx=Load(), lineno=1, col_offset=0, end_lineno=len(ls := fst_._lines),  # fst_.a because may have changed in Set processing
...
def _code_to_slice_mapping(
    ...
    fst_ = code_as(code, options, self.root._parse_params, coerce=fst.FST.get_option('coerce', options))
    ast_ = fst_.a
...
def _code_to_slice_BoolOp_values(
    ...
    coerce = fst.FST.get_option('coerce', options)
    fst_ = code_as_expr(code, options, self.root._parse_params, coerce=coerce)
    ast_ = fst_.a
...
def _code_to_slice_MatchSequence(
    ...
    coerce = fst.FST.get_option('coerce', options)
    fst_ = code_as_pattern(code, options, self.root._parse_params, coerce=coerce)
    ast_ = fst_.a
    is_delimited_matchseq = fst_.is_delimited_matchseq()
    ...
    if not one:
        ...
        elif one is False and isinstance(code, (str, list)):
            if is_delimited_matchseq:
                one = True
    ...
    if is_delimited_matchseq:  # delimited is restricted to the inside of the delimiters, which are removed
        fst_._trim_delimiters()
...
def _code_to_slice_MatchOr(
    ...
    try:
        fst_ = code_as_pattern(code, options, self.root._parse_params, coerce=coerce, allow_invalid_matchor=True)
...
def _code_to_slice__special1(
    ...
    ast_cls: type[AST],
    code_as: Callable,
    code_as_one: Callable,
) -> fst.FST | None:
    """Special slice which cannot convert multiple elements into one, `_type_params`, `_comprehensions` and
    `_aliases`."""
    ...
    fst_ = code_as_one(code, options, self.root._parse_params, coerce=coerce)

    ast = ast_cls([], lineno=1, col_offset=0, end_lineno=len(ls := fst_._lines),
                  end_col_offset=ls[-1].lenbytes)

    return fst.FST(ast, ls, None, from_=fst_, lcopy=False)._set_field([fst_.a], field, True, False)


def _code_to_slice__special2(
    self: fst.FST,
    code: Code | None,
    field: str,
    one: bool | None,
    ...
    """Very special slice with unique syntax, `_Assign_targets`, `_decorator_list` and `_comprehension_ifs`."""
    ...
    if codea_cls is ast_cls:  # if is already the wanted class and has a single element then don't convert to singleton sequence, makes more sense to use it as-is for these very special slices
        ...
        # field not length 1 so fall through to either coerce to Tuple or error
        ...
        try:
            ...
            if len(getattr(fst_.a, field)) != 1:
                raise NodeError(f'expecting single {ast_cls.__name__} element for put as `one=True`')
...
def _code_to_slice__Assign_targets(
    ...
    return _code_to_slice__special2(self, code, 'targets', one, options, _Assign_targets, code_as__Assign_targets,
                                    _code_as_one__Assign_targets)
...
def _code_to_slice__decorator_list(
    ...
    return _code_to_slice__special2(self, code, 'decorator_list', one, options, _decorator_list,
                                    code_as__decorator_list, _code_as_one__decorator_list)
...
def _code_to_slice__arglikes(
    ...
    if not one:
        fst_ = code_as__arglikes(code, options, self.root._parse_params, coerce=coerce)
    ...
    if codea_cls in ASTS_LEAF_EXPR:  # if putting expr as one then should be passed through as such
        if codea is not code:  # is FST
            ...
            fst_ = code_as_expr(code, options, self.root._parse_params)
            ast_ = fst_.a
        ...
        if codea is not code:  # is FST
            ...
            fst_ = code_as_keyword(code, options, self.root._parse_params)
            ast_ = fst_.a
        ...
        if len(arglikes) == 1:  # if is length 1 then maybe we put just this one element
            ...
            if arglike0.__class__ is keyword:
                raise NodeError('cannot put keyword into a Tuple for put as `one=True`')
    ...
    return fst.FST(ast, ls, None, from_=fst_, lcopy=False)._set_field([ast_], 'arglikes', True, False)


def _code_to_slice__comprehensions(
    self: fst.FST, code: Code | None, one: bool | None, options: Mapping[str, Any]
) -> fst.FST | None:
    return _code_to_slice__special1(self, code, 'generators', one, options, _comprehensions, code_as__comprehensions,
                                    code_as_comprehension)
...
def _code_to_slice__comprehensions_ifs(
    ...
    return _code_to_slice__special2(self, code, 'ifs', one, options, _comprehension_ifs, code_as__comprehension_ifs,
                                    _code_as_one__comprehension_ifs)
...
def _code_to_slice__aliases(
    ...
    return _code_to_slice__special1(self, code, 'names', one, options, _aliases, code_as__aliases, code_as_alias)


def _code_to_slice_Import_names(
    self: fst.FST, code: Code | None, one: bool | None, options: Mapping[str, Any]
) -> fst.FST | None:
    return _code_to_slice__special1(self, code, 'names', one, options, _aliases, code_as__Import_names,
                                   code_as_Import_name)
...
def _code_to_slice_ImportFrom_names(
    ...
    return _code_to_slice__special1(self, code, 'names', one, options, _aliases, code_as__ImportFrom_names,
                                   code_as_ImportFrom_name)
...
def _code_to_slice__withitems(
    ...
    if not one:
        fst_ = code_as__withitems(code, options, self.root._parse_params, coerce=coerce)
    ...
    if codea_cls in ASTS_LEAF_EXPR:  # if putting expr as one then should be passed through as such
        ...
        try:
            fst_ = code_as__withitems(code, options, self.root._parse_params, coerce=coerce or codea_cls is withitem)
            ...
            try:
                fst_ = code_as_expr(code, options, self.root._parse_params, coerce=coerce)  # source that may parse to a Tuple
            except Exception:
                raise exc from None

            if fst_.a.__class__ is not Tuple:
                raise
            ...
            for item in items:
                if item.optional_vars:
                    raise NodeError('cannot put withitem with optional_vars into a Tuple for put as `one=True`')
...
def _code_to_slice__type_params(
    ...
    return _code_to_slice__special1(self, code, 'type_params', one, options, _type_params, code_as__type_params,
                                    code_as_type_param)
...
def _code_to_slice__expr_arglikes(
    ...
    **Returns**:
    - `Tuple | List | Set`: For the functions which call this it doesn't matter the type, all they want is the `elts`.
    """
    ...
    if not one:
        fst_ = code_as__expr_arglikes(code, options, self.root._parse_params, coerce=coerce or codea_cls in ASTS_LEAF_LIST_OR_SET)  # the codea_cls sets coerce because we always want to accept List and Set as valid slices

        return fst_ if fst_.a.elts else None  # put empty sequence is same as delete

    if codea_cls in _ASTS_LEAF_EXPR_NO_TUPLE:  # if putting expr as one then most should be passed through as such except for Tuple, which may need arglike expressions inside it fixed
        if codea is not code:  # is FST
            fst_ = code
            ast_ = codea
        ...
        fst_ = code_as__expr_arglikes(code, options, self.root._parse_params, coerce=coerce, one=one)  # we use this instead of code_as_expr() because this field normally takes arglike-only expressions like '*not a' so if we fail to do so here it might seem strange
        ast_ = fst_.a
...
def _put_slice_ClassDef_bases(
    ...
    if keywords := ast.keywords:
        if body and keywords[0].f.loc[:2] < body[stop - 1].f.loc[2:] and stop:
            raise NodeError("cannot put to ClassDef.bases slice because it follows keywords, try the '_bases' field")
...
def _put_slice_Call_args(
    ...
    if keywords := ast.keywords:
        if body and keywords[0].f.loc[:2] < body[stop - 1].f.loc[2:] and stop:
            raise NodeError("cannot put to Call.args slice because it follows keywords, try the '_args' field")
...
def _put_slice_Call_ClassDef_keywords(
    ...
    if exprs and start != stop and body[start].f.loc < exprs[-1].f.loc:
        raise NodeError(f'cannot put to {ast.__class__.__name__}.keywords slice because it precedes {exprs_field}'
