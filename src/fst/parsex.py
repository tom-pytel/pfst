...
python SyntaxError exceptions do.
"""
...
def parse__Assign_targets(src: str, parse_params: Mapping[str, Any] = {}) -> AST:
    ...
    try:
        ast = _ast_parse1(f'_=\\\n{src_}\\\n_', parse_params)  # try assuming src has trailing equals

    except SyntaxError as bare_exc:
        try:
            ast = _ast_parse1(f'_=\\\n{src_}\\\n=_', parse_params)  # now check assuming src does not have trailing equals
...
def parse_comprehension(src: str, parse_params: Mapping[str, Any] = {}) -> AST:
    ...
    if ast.__class__ is not ListComp:
        raise ParseError(f'expecting comprehension, got {ast.__class__.__name__}')
...
def parse__comprehension_ifs(src: str, parse_params: Mapping[str, Any] = {}) -> AST:
    ...
    if ast.__class__ is not ListComp:
        raise ParseError(f'expecting comprehensions ifs, got {ast.__class__.__name__}')
