...
Care must be taken to ensure that invalid source doesn't get parsed into something unexpectedly valid due to the wrapper
source we use for the parsing.

The SyntaxError exceptions generated by this module do not contain the extended error location information that standard
python SyntaxError exceptions do. We also count on python SyntaxErrors being generated with locations to differentiate
these from our own for the logic flow.
"""
