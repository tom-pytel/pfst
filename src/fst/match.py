...
def _match_list(pat: type, tgt: _Targets, mstate: _MatchState) -> Mapping[str, Any] | None:
    ...
    if not isinstance(tgt, list):
        ...
        if not len(tgt):
            tgt = _EMPTY_LIST
        elif not tgt.is_item_FST:  # could be multi-node items like Dict/MatchMapping/arguments or Global/Nonlocal.names or MatchClass.kwd_attrs  # or isinstance(tgt[0], str) - should not be any strings from .at()
...
def subn(
    ...
    **Examples:**
    ...
    >>> res = f.copy().subn(pat, repl, nested=True, count=1)
    ...
    >>> res = f.copy().subn(pat, repl, loop=True)
    ...
    for m in gen:
        ...
        while True:  # for `loop`
            ...
            if loop is not False:
                if loop := loop - 1:
                    if m := replaced.match(pat):  # if `matched` somehow winds up deleted as None, the match will just fail
                        matched = replaced
    ...
    return self, (-count if count < 0 else count_start - count), total_count
