...
def add_lineconts(fst: FST) -> None:
    ...
    for f in (gen := fst.walk('loc')):
        ...
        if g is not None and f.parent is g.parent and (ln := g.end_ln) == f.ln:
            ...
            if (l := lines[ln])[col : col + 1].isspace() or (col and l[col - 1].isspace()):
                fst._put_src('\\\n', ln, col, ln, col, False)
...
def astbase(ast_cls: type[AST]) -> type[AST]:  # ast base class just above AST
    while (c := ast_cls.__bases__[0]) is not AST:
        ast_cls = c
...
class SliceExprlike(Fuzzy):
    ...
    def transfer_arguments(
        ...
        try:
            ...
            if cut:
                src.put_slice(slice_bck, src_start, src_start, field='_all', trivia=(False, False))
    ...
    def transfer_arglikes(
        ...
        try:
            ...
            if cut:
                src.put_slice(slice_bck, src_start, src_start, field=src_field, trivia=(False, False))
...
class Substitute(Fuzzy):
    ...
    name = 'sub'
    forever = True
  # (MExpr(value=M(value=...)), FST('__FST_value', Expr)),
    PATS_N_REPLS = {
        FunctionDef:      (MFunctionDef(decorator_list=M(decorator_list=...), name=M(name=...), args=M(args=...), returns=M(returns=...), body=M(body=...)), FST('@__FST_decorator_list\ndef __FST_name(__FST_args) -> __FST_returns: __FST_body', FunctionDef)),
        AsyncFunctionDef: (MAsyncFunctionDef(decorator_list=M(decorator_list=...), name=M(name=...), args=M(args=...), returns=M(returns=...), body=M(body=...)), FST('@__FST_decorator_list\nasync def __FST_name(__FST_args) -> __FST_returns: __FST_body', AsyncFunctionDef)),
        ClassDef:         (MClassDef(decorator_list=M(decorator_list=...), name=M(name=...), _bases=M(_bases=...), body=M(body=...)), FST('@__FST_decorator_list\nclass __FST_name(__FST__bases): __FST_body', ClassDef)),
        Return:           (MReturn(value=M(value=...)), FST('return __FST_value', Return)),
        Delete:           (MDelete(targets=M(targets=...)), FST('del __FST_targets', Delete)),
        Assign:           (MAssign(targets=M(targets=...), value=M(value=...)), FST('__FST_targets = __FST_value', Assign)),
        AugAssign:        (MAugAssign(target=M(target=...), value=M(value=...)), FST('__FST_target += __FST_value', AugAssign)),
        AnnAssign:        (MAnnAssign(target=M(target=...), annotation=M(annotation=...), value=M(value=...)), FST('__FST_target: __FST_annotation = __FST_value', AnnAssign)),
        For:              (MFor(target=M(target=...), iter=M(iter=...), body=M(body=...), orelse=M(orelse=...)), FST('for __FST_target in __FST_iter: __FST_body\nelse: __FST_orelse', For)),
        AsyncFor:         (MAsyncFor(target=M(target=...), iter=M(iter=...), body=M(body=...), orelse=M(orelse=...)), FST('async for __FST_target in __FST_iter: __FST_body\nelse: __FST_orelse', AsyncFor)),
        While:            (MWhile(test=M(test=...), body=M(body=...), orelse=M(orelse=...)), FST('while __FST_test: __FST_body\nelse: __FST_orelse', While)),
        If:               (MIf(test=M(test=...), body=M(body=...), orelse=M(orelse=...)), FST('if __FST_test: __FST_body\nelse: __FST_orelse', If)),
        With:             (MWith(items=M(items=...), body=M(body=...)), FST('with __FST_items: __FST_body', With)),
        AsyncWith:        (MAsyncWith(items=M(items=...), body=M(body=...)), FST('async with __FST_items: __FST_body', AsyncWith)),
        Match:            (MMatch(subject=M(subject=...), cases=M(cases=...)), FST('match __FST_subject:\n    case "...": __FST_cases', Match)),
        Raise:            (MRaise(exc=M(exc=...), cause=M(cause=...)), FST('raise __FST_exc from __FST_cause', Raise)),
        Try:              (MTry(body=M(body=...), handlers=M(handlers=...), orelse=M(orelse=...), finalbody=M(finalbody=...)), FST('try: __FST_body\nexcept "...": __FST_handlers\nelse: __FST_orelse\nfinally: __FST_finalbody', Try)),
        Assert:           (MAssert(test=M(test=...), msg=M(msg=...)), FST('assert __FST_test, __FST_msg', Assert)),
        Import:           (MImport(names=M(names=...)), FST('import __FST_names', Import)),
        ImportFrom:       (MImportFrom(module=M(module=...), names=M(names=...)), FST('from .__FST_module import __FST_names', ImportFrom)),
        Global:           (MGlobal(names=M(names=...)), FST('global __FST_names', Global)),
        Nonlocal:         (MNonlocal(names=M(names=...)), FST('nonlocal __FST_names', Nonlocal)),
        Expr:             (MExpr, FST('__FST_', Expr)),
        Pass:             (MPass(), FST('__FST_', Name)),
        Break:            (MBreak(), FST('__FST_', Name)),
        Continue:         (MContinue(), FST('__FST_', Name)),
        BoolOp:           (MBoolOp(values=M(values=...)), FST('__FST_values', Name)),
        NamedExpr:        (MNamedExpr(target=M(target=...), value=M(value=...)), FST('__FST_target := __FST_value', NamedExpr)),
        BinOp:            (MBinOp(left=M(left=...), right=M(right=...)), FST('__FST_left + __FST_right', BinOp)),
        UnaryOp:          (MUnaryOp(operand=M(operand=...)), FST('-__FST_operand', UnaryOp)),
        Lambda:           (MLambda(args=M(args=...), body=M(body=...)), FST('lambda __FST_args: __FST_body', Lambda)),
        IfExp:            (MIfExp(body=M(body=...), test=M(test=...), orelse=M(orelse=...)), FST('__FST_body if __FST_test else __FST_orelse', IfExp)),
        Dict:             (MDict(_all=M(_all=...)), FST('{"...": __FST__all}', Dict)),
        Set:              (MSet(elts=M(elts=...)), FST('{__FST_elts}', Set)),
        ListComp:         (MListComp(elt=M(elt=...), generators=M(generators=...)), FST('[__FST_elt for __FST_generators in "..."]', ListComp)),
        SetComp:          (MSetComp(elt=M(elt=...), generators=M(generators=...)), FST('{__FST_elt for __FST_generators in "..."}', SetComp)),
        DictComp:         (MDictComp(key=M(key=...), value=M(value=...), generators=M(generators=...)), FST('{__FST_key: __FST_value for __FST_generators in "..."}', DictComp)),
        GeneratorExp:     (MGeneratorExp(elt=M(elt=...), generators=M(generators=...)), FST('(__FST_elt for __FST_generators in "...")', GeneratorExp)),
        Await:            (MAwait(value=M(value=...)), FST('await __FST_value', Await)),
        Yield:            (MYield(value=M(value=...)), FST('yield __FST_value', Yield)),
        YieldFrom:        (MYieldFrom(value=M(value=...)), FST('yield from __FST_value', YieldFrom)),
        Compare:          (MCompare(_all=M(_all=...)), FST('__FST__all', Name)),
        ...
        Name:             (MName(id=M(id=...)), FST('__FST_id', Name)),
        List:             (MList(elts=M(elts=...)), FST('[__FST_elts]', List)),
    ...
    if PYGE12:
        PATS_N_REPLS.update({
            ...
            TypeAlias:        (MTypeAlias(name=M(name=...), type_params=M(type_params=...), value=M(value=...)), FST('type __FST_name[__FST_type_params] = __FST_value', TypeAlias)),
            TypeVar:          (MTypeVar(name=M(name=...), bound=M(bound=...)), FST('__FST_name: __FST_bound', TypeVar)),
            ParamSpec:        (MParamSpec(name=M(name=...)), FST('**__FST_name', ParamSpec)),
            TypeVarTuple:     (MTypeVarTuple(name=M(name=...)), FST('*__FST_name', TypeVarTuple)),
        })

    if PYGE13:
        PATS_N_REPLS.update({
            TypeVar:          (MTypeVar(name=M(name=...), bound=M(bound=...), default_value=M(default_value=...)), FST('__FST_name: __FST_bound = __FST_default_value', TypeVar)),
            ParamSpec:        (MParamSpec(name=M(name=...), default_value=M(default_value=...)), FST('**__FST_name = __FST_default_value', ParamSpec)),
